...
namespace PackageGenerationTool
{
    ...
    public class PackageGenerationToolEditor : EditorWindow
    {
        ...
        public static void ShowMyEditor()
        {
            ...
            window = GetWindow<PackageGenerationToolEditor>();
        ...
        public void CreateGUI()
        {
...

            {
                ...
                dependencies_list.itemsSource = items;
        ...
        private void OnRefreshPackagesClicked()
        {
            ...
            IList<string> deps = dependencies_list.itemsSource as IList<string>;

            foreach (PackageInfo pi in remote_packages)
            {
                if (deps.Contains(pi.name))
                {
                    continue;
                }
                else
                {
                    dependencies_list.itemsSource.Add(pi.name);
                }
            }
        }
        ...
        private void OnGenerateClicked()
        {
            ...
            List<string> dependency_pkg_list = new List<string>();
            IList<string> deps = dependencies_list.itemsSource as IList<string>;
            foreach (int pkg_index in dependencies_list.selectedIndices)
            {
                dependency_pkg_list.Add(deps[pkg_index]);
            }
            IEnumerable<object> selected_pkgs = dependencies_list.selectedItems;
            ...
            SerializablePackageInfo spi = new SerializablePackageInfo()
            {
                ...
                Dependencies = GeneratePackageDependencyList(dependency_pkg_list)
            };
        ...
        private List<SerializableDependencyInfo> GeneratePackageDependencyList(List<string> package_list)
        {
            List<SerializableDependencyInfo> package_dependencies = new List<SerializableDependencyInfo>();
            foreach(string package_name in package_list)
            {
                SearchRequest search_req = Client.Search(package_name);
                while (!search_req.IsCompleted)
                {}

                SerializableDependencyInfo dependency = new SerializableDependencyInfo()
                {
                    name = package_name,
                    version = search_req.Result[0].version
                };
